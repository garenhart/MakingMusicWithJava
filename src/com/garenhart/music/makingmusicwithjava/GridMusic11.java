...
public class GridMusic11 implements JMC {
    ...
    * Return a part with the necessary modification ready for playback.
	...
	// apply randomness to note dynamic for each phrase
	for(int i=0; i<temp.size();i++) {
	    Phrase phr = temp.getPhrase(i);
	    for (int j=0; j<phr.size(); j++) {
		Note n = phr.getNote(j);
		n.setDynamic(phr.getVolume() + (int)(Math.random() * 40) - 20);
	    }
	}
	return temp;
}
    ...
    public void samplePlay() {
        // set up instruments
        Instrument hats = new SampleInst("data/Hats.au"); //Hats.au");
        hats.setOutput(Instrument.REALTIME);
